...
all_keywords_str = ', '.join([f"{keyword}({count})({status})" for keyword, count in keyword_counts.items()])
...
with open(md_file_path, 'w') as md:
        ...
        for keyword in top_5_keywords:
            if keyword in row['Keywords']:
                ...
                articles_by_keyword[keyword].append({
                    ...
                    'author': row.get('Author', 'No Author'),
    ...
    for keyword, articles, in articles_by_keyword.items():
        ...
        for article in articles:
            ...
            md.write(f"1. <a href='{permalink}'>{title}</a>\n")
