...
for idx, keyword in enumerate(top_5_keywords):
    ...
    # all_keywords_str += f"{keyword}{count}{status}, "  # Append to the existing string
...
with open(md_file_path, 'w') as md:
    ...
    all_keywords_str_md = ', '.join([f"{keyword} (**{count}**) {status}" for keyword, count in keyword_counts.items()])
    # print("All Keywords and Their Frequencies (Markdown):", all_keywords_str_md)
    
    md.write("\n**All Keywords and Their Frequencies**\n\n ")
    md.write(f"{status} {all_keywords_str_md} \n\n")
